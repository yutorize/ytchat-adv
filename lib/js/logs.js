...
function bgMusicSet(){
  ...
  bgmBorderSet();
  if(!bgmExist){
    document.querySelector('#options dt:first-of-type').style.display = 'none';
    document.querySelector('#options dd:first-of-type').style.display = 'none';
  }
  else {
...
function bgmChange(num){
  if(num != currentBgm){
    ...
    bgMusic.volume = (Number(bgMusics[num]['vol']) || 100) / 100 * soundVolume;
