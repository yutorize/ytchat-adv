...
document.getElementById('option-view-mode').addEventListener('change', function(e) {
  ...
  if(type === 'lite'){
    ...
    document.getElementById('yt-api-cdn').remove();
    bgImages = [];
    bgBorders = [];
    currentBg = 0;
    ytPlayerReadyOk = 0;
...
async function downloadAllLogs(evt){
  ...
  const pageMaxNum = Number(pageMax) || 0;
  if(pageMaxNum){
    for(let page = 1; page <= pageMaxNum; page++){
      zip.file('log'+page+'.html', await JSZipUtils.getBinaryContent(url+'&page='+page));
    };
  }
  else { zip.file('log.html', await JSZipUtils.getBinaryContent(url)); }
