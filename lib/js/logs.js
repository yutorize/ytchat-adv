...
function pageSet(set) {
  if     (set ==  'all'){ currentPage=0; pageTurnSet(); location.hash = 'all'; window.scrollTo(0,0); }
  else if(set == 'next'){ currentPage++; pageTurnSet(); location.hash = 'page-'+currentPage; window.scrollTo(0,0); }
  else if(set == 'prev'){ currentPage--; pageTurnSet(); location.hash = 'page-'+currentPage; window.scrollTo(0,0); }
  ...
  //document.querySelectorAll('#foot-area .credit').forEach(obj => {
  //  obj.style.display = (currentPage == i) ? '' : 'none';
  //})
}
function pageTurnSet() {
  pageTurning = 1;
  window.setTimeout(()=>{
    pageTurning = 0;
  }, 1000);
...
function anchorMove(id){
  ...
  if(id === 'all'){
    ...
    const target = document.getElementById(id).closest('.logs') || document.querySelector(`.logs#${id}`) || document.querySelector(`#credit-area`);
    if(target && target.id == 'credit-area'){
      target.querySelectorAll('.credit').forEach(obj => { obj.classList.remove('view') })
...
function bgBorderSet(){
  ...
  document.querySelectorAll('.bg-border').forEach(elm => {
    ...
    const value = bgImages[num]['page'] < currentPage ? 0
                : bgImages[num]['page'] > currentPage ? Infinity
...
function bgmBorderSet(){
  ...
  document.querySelectorAll('.bgm-border').forEach(elm => {
    ...
    const value = bgMusics[num]['page'] < currentPage ? 0
                : bgMusics[num]['page'] > currentPage ? Infinity
                : (elm.getBoundingClientRect().top + document.scrollingElement.scrollTop);
...
function bgScroll(){
  ...
  if (pageTurning){
    if(document.scrollingElement.scrollTop < center) { pageTurning = 0; }
    return;
  }
  // 背景
