...
function fontFamilyChange(font){
  const logs = document.querySelectorAll('.logs-font, .form-comm');
  for(let i=0; i<logs.length; i++){
  ...
  for (let i in setStatus){
    statusRemoconArea += `
            <li class="dice-button">
              ...
              <input type="text" class="form-comm" id="edit-stt-${unitId}-${setStatus[i]}-value" data-comm-pre="@${setStatus[i]}" data-palette-target='${unitNameEscaped}' placeholder="Enterで送信">
            </li>`;
  }
...
$(document).on("keydown", ".form-comm", function(e) {
  ...
  // 前回送信した発言を取得
    ...
    if(!$(e.target).val() && beforeComm[e.target.id] && !e.ctrlKey) {
      e.preventDefault();
