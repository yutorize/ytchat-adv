...
function boxOpen(id,details){
  ...
  if (boxObj.classList.contains('open') && !boxObj.hasAttribute('style')) {
    boxObj.style.width = `${boxObj.clientWidth}px`;
    boxObj.style.height = `${boxObj.clientHeight}px`;
  }
}
...
function tabAdd(tabNum){
  ...
  newTab.innerHTML = `
    ...
    `;
  let targetGroup;
  if(window.matchMedia('(mix-width:601px)').matches && roomConfig.tab && roomConfig.tab[tabNum-1]){
