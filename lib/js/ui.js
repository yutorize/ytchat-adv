...
function npcBoxSet(){
  ...
  select.size = (n < 2) ? 2 : (n > 10) ? 10 : n; //高さ
}
// 名前変更
...
function unitAdd(unitName){
  ...
  for(let i in setStatus) {
    newUnit.innerHTML += `<dd id="stt-${unitId}-${setStatus[i]}" style="display:${unitList[unitName]['status'][setStatus[i]] ? 'block' : 'none'}"><dl><dt>${setStatus[i]}</dt><dd id="stt-${unitId}-${setStatus[i]}-value"></dd></dl></dd>`;
  }
