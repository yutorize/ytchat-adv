...
// 音量調節
function volumeSet(id){ console.log(id)
  const volumeValue = localStorage.getItem(id);
  document.getElementById(id).value             = (volumeValue ? volumeValue : 80) + '%';
  document.getElementById(id+"-view").innerHTML = (volumeValue ? volumeValue : 80) + '%';
}
$(function($){
  $(document).on("change", '#config-sound input[type="range"]', function(e) {
    ...
    let flush
    if(obj.id === 'volume-chat'){ flush = new Audio(chatSE); }
    flush.volume = obj.value / 100;
    flush.play();
