...
function scrollBottom(tabId){
  ...
  tab.lastElementChild.scrollIntoView(false);
...
function fontFamilyChange(font){
  const logs = document.getElementsByClassName('logs-font');
  for(let i=0; i<logs.length; i++){
    logs[i].style.fontFamily = '"Lato",'+font+',"UD Digi Kyokasho NP-R","Meiryo","YuKyokasho Yoko"';
  }
  fontFamily = font;
...
function diceAdd(){
  ...
  newDice.innerHTML = `<textarea class="form-comm" id="dice-button-${n}" data-lock="full" data-num="${n}" rows="1"></textarea><i onclick="lockTypeChange(${n});"></i><button onclick="formSubmit('dice-button-${n}');"></button></div>`;
  ...
  if(!diceForms[n]){ diceForms.push({"type":"full","value":"2d6"}); }
  diceSave();
}
function diceDel(){
  ...
  for (let i in setStatus){
    statusRemoconArea += `
            <li class="dice-button">
              ...
              <input type="text" class="form-comm" id="edit-stt-${unitId}-${n}-${setStatus[i]}-value" data-comm-pre="@${setStatus[i]}" data-part="${unitId}-${n}" data-palette-target='${unitNameEscaped}'>
            </li>`;
  }
  statusRemoconArea += `
            <li class="dice-button">
