...
function diceAdd(area){
  ...
  newDice.innerHTML = `<textarea class="form-comm" id="dice-button-${area}-${n}" data-lock="${diceDefType[area]}" data-area="${area}" data-num="${n}" rows="1"></textarea><i onclick="lockTypeChange(${area},${n});"></i><button data-id="dice-button-${area}-${n}"></button></div>`;
...
ダブルクリックで即送信`;
  ...
  for (let i in setStatus){
    statusRemoconArea += `
            <li class="dice-button">
              ...
              <textarea class="form-comm" name="stt-${setStatus[i]}" id="edit-stt-${unitId}-${setStatus[i]}-value" data-comm-pre="@${setStatus[i]}" data-palette-target='${unitNameEscaped}' rows="1" placeholder="Enterで送信"></textarea>
            </li>`;
  }
      ...
      <div class="sheet-body">
        <div class="status-remocon-area">
          ...
          <ul>${statusRemoconArea}
            <li class="dice-button big">
              <button onclick="formSubmit('edit-stt-${unitId}-memo-value','${unitNameEscaped}');">ﾒﾓ</button>
              <textarea class="form-comm" name="stt-memo" id="edit-stt-${unitId}-memo-value" data-comm-pre="@memo" data-palette-target='${unitNameEscaped}' rows="1" placeholder="Enterで送信／計算しない">${unitList[unitName]['memo'].replace(/<br>/g,"\n")}</textarea>
            </li>
  ...
  for (let i in setStatus){
    autosize(document.getElementById(`edit-stt-${unitId}-${setStatus[i]}-value`));
  }
  autosize(document.getElementById(`edit-stt-${unitId}-memo-value`));
}
// ステータス伸縮 ----------------------------------------
let statusRowWidth = 6;
function statusScale(num){
  if(num){ statusRowWidth = num }
  else { statusRowWidth = statusRowWidth <= 4 ? 10 : statusRowWidth-1; }
...
$(function($){
  $(document).on("change", '#config-sound input[type="range"]', function(e) {
    ...
    if(type === 'bgm' || type === 'master') {
      ...
      let se;
      if     (type === 'chat') { se = chatSE; }
      else if(type === 'mark') { se = markSE; }
      else if(type === 'ready'){ se = readySE; }
      else { se = chatSE; }
