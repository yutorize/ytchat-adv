...
function bgmVolumeChange(){
  bgMusic.volume = currentBgmVol * (volumes['bgm'] / 100) * (volumes['master'] / 100);
  console.log('BGM-vol:'+bgMusic.volume);
}
...
// ボリューム変更
function bgmPreviewVol(volume){
  audioPreview.volume = ((volume || 100) / 100) * (volumes['bgm'] / 100) * (volumes['master'] / 100);;
  document.getElementById('bgm-set-volume-text').innerHTML = volume;
}
// 履歴追加
