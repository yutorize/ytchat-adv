...
function scrollBottom(tabId){
  ...
  const list    = document.querySelector('#tablist-tab'+tabId);
  tab.scrollTop = tab.scrollHeight;
  tabname.dataset.unread = 0;
  notice.dataset.unread = 0;
  list.dataset.unread = 0;
}
// ログのスクロール位置を確認
...
function diceScale(area, num){
  ...
  autosizeUpdate(document.querySelectorAll('#main-form #dice-button-'+area+' textarea'));
...
function tabAdd(tabNum){
  ...
  newTab.innerHTML = `
    ...
    <div class="logs logs-font" id="chat-logs-tab${tabNum}" data-unread="0" style="font-size:${setFontSize}%;"></div>
    ...
    <div class="option"><input class="option-fontsize-tab" type="range" min="80" max="120" step="1" value="${setFontSize}"></div>
