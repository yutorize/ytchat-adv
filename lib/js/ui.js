...
function layoutChange(){
  config.setSheet  = document.getElementById('window-layout-sheet').value;
  config.setSide   = document.getElementById('window-layout-sidebar').value;
  const base      = document.getElementById('base');
  const sidebar   = document.getElementById('sidebar');
  const sheet     = document.getElementById('sheet-area');
  const topicEdit = document.getElementById('edit-topic');
  if(config.setSheet === 'R'){
    sheet.classList.remove('left');
    if(config.setSide === 'right-top'){
    ...
    else if(config.setSide === 'right-bottom'){
      base.style.gridTemplateColumns = '1fr max-content max-content';
      base.style.gridTemplateAreas   = `
        " menu  menu sheet"
        " chat  side sheet"
    ...
    else if(config.setSide === 'left-top'){
    ...
    else if(config.setSide === 'left-bottom'){
      base.style.gridTemplateColumns = 'max-content 1fr max-content';
      base.style.gridTemplateAreas   = `
        " menu  menu sheet"
        " side  chat sheet"
    ...
    else if(config.setSide === 'shallow'){
      base.style.gridTemplateColumns = '1fr max-content';
      base.style.gridTemplateAreas   = `
        " menu  sheet"
        " chat  sheet"
  ...
  else if(config.setSide.match(/bottom/)){
...
function tabAdd(tabNum){
  ...
  newTab.innerHTML = `
    ...
    `;
  let targetGroup;
  if(roomConfig.tab && roomConfig.tab[tabNum-1]){
