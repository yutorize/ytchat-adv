  ...
  .then(data => {
    ...
    data['logs'].forEach( (value, index, array) => {
      ...
      if(value['comm']){
        ...
        if(!targetDl.classList.contains('system')){ newComm.innerHTML += `<button class="rewrite" onclick="rewriteOpen(${value['num']})"></button>` }
      ...
      if(value['info']){
        ...
        if(value['system']){
          ...
          if(value['system'].match(/topic/)) { value['info'] = tagConvert(value['info']) }
        }
...
function tagConvert (comm){
  ...
  comm = comm
    .replace(/((?:\G|>)[^<]*?)(https?:\/\/[^\s\<]+)/gi,'$1<a href="$2" target="_blank">$2</a>')
