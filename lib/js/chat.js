...
function logGet(){
  ...
  .done( (data) => {
    ...
    data.forEach( (value, index, array) => {
      ...
      // コメント処理
      if(value['comm']){
        if(!romMode){ [soundFlag['mark'] , value['comm']] = wordMark(value['comm']); }
  ...
  for (let name in unitList) {
    ...
    for (let i in setStatus) {
      ...
      if(value && value.match(/[,、]/)){
        value = '<span>'+value.split(/[,、]/).join(',</span><span>', )+'</span>'; }
      document.getElementById('stt-'+id+'-'+stt+'-value').innerHTML = value;
      // 値が入ってない項目は非表示
...
function statusGet(name){
  ...
  if(unitList[name]){
    for(let i in setStatus) {
      text += setStatus[i] +'='+ unitList[name]['status'][setStatus[i]] + '<>';
    }
  }
  return text;
}
