  ...
  .done( (data) => {
    ...
    data.forEach( (value, index, array) => {
      ...
      if(value['comm']){
        ...
        //newComm.style.opacity = 0;
        targetTab.lastElementChild.appendChild(newComm);
        //setTimeout(function(){ newComm.style.opacity = 1; }, 20);
        soundFlag['normal'] = 1;
      }
      if(value['info']){
        ...
        //setTimeout(function(){ newInfo.style.opacity = 1; }, 20);
        soundFlag['normal'] = 1;
      }
...
function statusUpdate () {
  for (let name in unitList) {
    ...
    if(unitList[name]['status']['check']){ document.getElementById('stt-unit-'+id).classList.add('check'); }
...
// 送信
function commSend(comm,tab,name,color,address,stt,bcdice){
...
function formSubmit(objId,unitName){
  ...
  if(bcdiceAPI){
    ...
    if(hit){
      ...
      .always( (data) => {
        commSend(comm, target, name, color, address, stt, bcdice);
      });
    }
    else { commSend(comm, target, name, color, address, stt); }
  }
  // 通常送信処理
  else {
    commSend(comm, target, name, color, address, stt);
  }
}
//名前・ステータスコマンドチェック
let nameToColor = {};
function nameCheck(name, comm){
  ...
  if(newComm.match(/^(ER|RE)[0-9]*(\s|$)/i)){ stt = statusGet(name); }
  return [name, newComm, stt];
}
function statusGet(name){
  if(unitList[name] && unitList[name]['status']){
    return unitList[name]['status'];
  }
}
// BCDice-APIに送信
