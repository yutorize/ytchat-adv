...
window.onload = function(){
  const savedName = localStorage.getItem(roomId+'-name');
  userId = localStorage.getItem('userid');
  if(!userId){
    userId = randomId(7); //ランダムID付与
...
function enterRoom() {
  ...
  if(name === ''){ return console.log('名前が入力されていません'); }
  $.ajax({
    url: cgiPath,
    type: 'POST',
    cache: false,
...
function logGet(){
  ...
  .done( (data) => {
    data.forEach( (value, index, array) => {
      const scrollOK = scrollCheck('chat-logs-tab'+value['tab']);
      ...
      if(value['info']){
        ...
        if(value['game']){ newInfo.dataset.game = value['game']; }
        if(value['crit']){ newInfo.dataset.crit = value['crit']; }
      ...
      if(scrollOK) { scrollBottom('chat-logs-tab'+value['tab']); }
    });
  })
  .fail( (data) => {
...
function commSend(comm,tab,name,color,stt){
  if(name === '' || name === undefined){ return console.log('送信する名前がありません'); }
  if(comm === '' || comm === undefined){ return console.log('送信するテキストがありません'); }
  if(nameList[0] === undefined){ return console.log('入室していません'); }
  $.ajax({
    url: cgiPath,
    type: 'POST',
    cache: false,
