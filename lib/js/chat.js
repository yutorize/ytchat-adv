    ...
    data.forEach( (value, index, array) => {
      ...
      if(value['system']){
        ...
        // 画像
          ...
          value['info'] = `<img src="${url}" ${ scrollOK ? 'onload="scrollBottom('+value['tab']+')"': '' }>`;
        }
        // BGM
...
function statusUpdate () {
  for (let name in unitList) {
    ...
    document.getElementById('stt-memo-'+id).innerHTML = unitList[name]['memo'];
...
function nameCheck(name, comm){
  ...
  // コメントチェック
  let reg = new RegExp("^("+names.join('|')+")?[@＠]((?:"+setStatus.join('|')+")|メモ|memo|check|delete)?");
...
document.getElementById('sheet-area').addEventListener("click",function(e){
  if (e.target.matches("select.chat-palette option")) {
    const name = htmlEscape( e.target.parentNode.dataset.name );
