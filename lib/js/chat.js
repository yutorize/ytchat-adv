...
window.onload = function(){
  ...
  .done( (data) => {
    ...
    const savedMarkName = localStorage.getItem('markName');
    markName = savedMarkName ? savedMarkName : 1;   
    ...
    const savedExceptList  = JSON.parse(localStorage.getItem('exceptList'));
    exceptList = savedExceptList ? savedExceptList  : [];
    ...
    if(savedDiceForms) { diceForms  = savedDiceForms; }
    //入室済み
...
function roomLoad(re){
  ...
  .done( (data) => {
    ...
    if(!romMode){
      ...
      //ダイス
      let diceNum0; let diceNum1;
      try { diceNum0 = diceForms[0].length ? diceForms[0].length : 1; } catch(e){ diceNum0 = 1 }
      try { diceNum1 = diceForms[1].length ? diceForms[1].length : 6; } catch(e){ diceNum1 = 6 }
