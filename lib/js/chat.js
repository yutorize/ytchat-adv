...
function logGet(){
  ...
  .done( (data) => {
    ...
    // 着信音声
    if(sound){
      const flush = new Audio(sound);
      flush.volume = document.getElementById("volume-chat").value / 100;
      flush.play();
    }
  })
  .fail( (data) => {
...
function nameCheck(name, comm){
  ...
  // コメントチェック
  let stt = '';
  let reg = new RegExp("^("+names.join('|')+")?[@＠]((?:"+setStatus.join('|')+")|delete)?");console.log(reg);
