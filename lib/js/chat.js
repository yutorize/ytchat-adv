...
function roomLoad(re){
  ...
  .done( (data) => {
    ...
    //BGM取得
    if(data['bgm']) {
      document.getElementById('bgm-title').innerHTML = `<a class="link" onclick="boxOpen('bgm-confirm')">${data['bgm']['title']}</a>`;
      bgMusic.src = data['bgm']['url'];
      currentBgmVol = (Number(data['bgm']['vol']) / 100);
      bgmVolumeChange();
      boxOpen('bgm-confirm');
    }
...
function logGet(){
  ...
  .done( (data) => {
    ...
    data.forEach( (value, index, array) => {
      ...
      if(beforeLastnumber && value['system']){
        ...
        //BGM処理
          ...
          let bgmUrl; let bgmTitle; let bgmVolume;
          if(value['system'].match(/^bgm:([0-9]+):(.+)$/)){
            bgmUrl   = RegExp.$2;
            bgmVolume= Number(RegExp.$1);
            bgmTitle = ` <a class="link" onclick="boxOpen('bgm-confirm')">${value['info']}</a>`;
            bgmHistory[bgmUrl] = [ value['info'] , bgmVolume ];
            bgmHistoryUpdate();
          }
          else {
            bgmUrl = '';
          ...
          document.getElementById('bgm-title').innerHTML = bgmTitle;
          currentBgmVol = (bgmVolume || 100) / 100;
          bgmVolumeChange();
          bgMusic.src = bgmUrl;
          bgMusic.play();
        }
      ...
      if(value['system']){
        ...
        // レディチェック
          ...
          value['info'] = `<button onclick="checkSubmit(1)">✔準備ＯＫ！</button><button onclick="checkSubmit(0)">×まだ待って</button>`;
        }
      }
      // ユニット色更新
      if (unitList[value['name']]){
    ...
    if(beforeLastnumber && Object.keys(soundFlag).length > 0){
      ...
      if(master > 0){
        let sound; let vol = 0;
        if     (soundFlag['ready']){ sound = readySE; vol = volumes['ready']; }
        else if(soundFlag['mark'] ){ sound = markSE;  vol = volumes['mark']; }
        if(vol < 1){
          sound = chatSE;
          vol = volumes['chat'];
        }
