...
function tagConvert (comm){
  ...
  Object.keys(tooltips).forEach(key => {
    if (!key) return;
    if(comm.match(key)){
      comm = comm.replace(new RegExp(key,'g'), (word) => {
        tooltipSet.push( { 'word':word, 'tip':tooltips[key] } );
        return `<!tooltip#${tooltipSet.length}>`;
      });
    }
  });
  comm = comm.replace(/<!tooltip#([0-9]+)>/gi, (all,num) => {
    return `<span class="tooltip" onmouseover="tooltipHover(event)">${tooltipSet[Number(num)-1]['word']}<span class="tooltip-text">${tooltipSet[Number(num)-1]['tip']}</span></span>`;
  })
